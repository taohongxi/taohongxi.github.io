d3.csv("WhatsgoodlyData-6.csv").then(function(data) {
 ...
 var formattedData = {
  ...
  function sortDataByColor(data){
    ...
    var temp =data.sort(function(a, b) {
      return colorMapOrder[a.color] - colorMapOrder[b.color];
    });
    // count the number of occurrences of each color in data
    const colorCounts = Object.values(data.reduce((acc, {color}) => {
      acc[color] = (acc[color] || 0) + 1;
      return acc;
    }, {}));
    console.log(colorCounts)
    const groupedDatatemp = temp.reduce((acc, cur) => {
    ...
    const groupedDataSorted = Object.fromEntries(
      Object.entries(groupedDatatemp).sort((a, b) =>  a[1].length - b[1].length)
    );
    const combinedArray = Object.values(groupedDataSorted).reduce((acc, cur) => acc.concat(cur), []);
    return combinedArray
  }
    

   
  formattedData.children[3].children = sortDataByColor(formattedData.children[3].children)
  formattedData.children[4].children = sortDataByColor(formattedData.children[4].children)
 ...
 var description = countGroup.append("text")
 .attr("font-size", "16px")
 .attr("fill", "black")
